...
MainMenuState::MainMenuState(){
	...
	//Ladataan/luodaan asetukset, jos niit‰ ei ole.
...
void MainMenuState::runMainMenuState(){
	while (menuModeIsActive){
		...
		handleControls();
		if (showOptions){	//Jos halutaan n‰ytt‰‰ asetusruutu, niin piirret‰‰n ja p‰ivitet‰‰n niit‰.
			updateOptionsMenuState();
			drawOptionsMenuState();
		}
		else{				//Jos ei, niin p‰ivitet‰‰n p‰‰valikkoa
			updateMainMenuState();
			drawMainMenuState();
		}
...
void MainMenuState::updateMainMenuState(){
	...
	if (titleAnimation < 255){
		titleAnimation += (4 * dt);
		title.setFillColor(Color(255, 255, 255, titleAnimation));
	}
}
...
//Vaihdetaan ruudunp‰ivitys rajoitinta
...
//Vaihtaa ikkunan tilaa t‰yteen ruutuun tai takaisin ikkuna tilaan.
...
//K‰sitell‰‰n mit‰ n‰pp‰imi‰ k‰ytt‰j‰ syˆtt‰‰
