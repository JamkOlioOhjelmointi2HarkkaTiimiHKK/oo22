...
void EditorState::update()
{
	...
	numberText.setPosition((300 + 380) / 2 - numberText.getGlobalBounds().width / 2, 690); // asetetaan tiedoston numero n‰pp‰inten v‰liin
	numberText.setString(std::to_string(numberOfPart));
}
...
void EditorState::save()
{
	...
	if (std::stoi(temp) < numberOfPart)
	{
		...
		facts << numberOfPart; //kerrotaan monesko mapPartti on luotu viimeksi ja samaan aikaan montako niit‰ on
	}
